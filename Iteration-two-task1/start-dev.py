...
TESTER_DIR = PROJECT_ROOT / "agent-tester"
...
def main() -> None:
    ...
    if not TESTER_DIR.exists():
        print(f"[错误] Agent Tester 目录不存在: {TESTER_DIR}", flush=True)
        sys.exit(1)
    ...
    processes["agent-tester"] = start_service(
        "agent-tester",
    ...
    print("    - Agent测试:  http://localhost:8080", flush=True)
    print("-" * 60, flush=True)
