...
from app.core.agent.tool_validation import validate_style
...
def _parse_detection_result(text: str) -> list[dict[str, str]]:
    """Parse JSON array of foreshadowing detections from LLM response."""
    import re as _re
    ...
    match = _re.search(r"```(?:json)?\s*(\[[\s\S]*?\])\s*```", text, _re.DOTALL)
    if match:
        try:
...
def create_world_tools(project_id: str) -> list[Any]:
    ...
    def create_world_setting(name: str, content: str) -> str:
        ...
        # Pre-validation: check for style conflicts
        style_warning = validate_style(project_id, f"{name} {content}")
        warning_prefix = f"{style_warning}\n\n" if style_warning else ""
        ...
        try:
            ...
            asyncio.run(_create())
            return f"{warning_prefix}Successfully created world setting '{name}' (ID: {world_setting_id})."
...
def create_plot_tools(project_id: str) -> list[Any]:
    ...
    @tool
    def check_timeline_conflict(event_a: str, event_b: str, chapter_a: str = "", chapter_b: str = "") -> str:
        """Detect timeline conflicts between two story events.

        Use this when you need to verify that two events can logically coexist
        in the story timeline.

        Args:
            event_a: Description of the first event
            event_b: Description of the second event
            chapter_a: Chapter or position reference for the first event (e.g. "第3章" or "chapter 3")
            chapter_b: Chapter or position reference for the second event

        Returns:
            Conflict analysis: whether the events are consistent in the timeline
        """
        try:
            ...
            async def _check() -> str:
                async with get_db() as db:
                    ...
                    outlines = await service.get_all_outlines(project_id)

                    chapter_map: dict[str, int] = {}
                    for ch in chapters:
                        key = str(ch.get("chapter_number", ""))
                        if key:
                            chapter_map[key] = int(key)
                    outline_map: dict[str, int] = {}
                    for ol in outlines:
                        if ol.get("sort_order") is not None:
                            outline_map[ol["id"]] = int(ol["sort_order"])

                def _extract_chapter_num(ref: str) -> int | None:
                    import re

                    for pattern in [r"第(\d+)章", r"chapter\s*(\d+)", r"ch(\d+)", r"(\d+)"]:
                        m = re.search(pattern, ref, re.IGNORECASE)
                        if m:
                            return int(m.group(1))
                    return None

                def _find_position(desc: str, chapter_ref: str) -> str:
                    pos = _extract_chapter_num(chapter_ref) if chapter_ref else None
                    if pos:
                        ch = chapter_map.get(str(pos))
                        if ch is not None:
                            return f"Chapter {ch}"
                        return f"Unknown chapter ({chapter_ref})"
                    return "Unknown position (no chapter reference)"

                pos_a = _find_position(event_a, chapter_a)
                pos_b = _find_position(event_b, chapter_b)
                num_a = _extract_chapter_num(chapter_a) if chapter_a else None
                num_b = _extract_chapter_num(chapter_b) if chapter_b else None

                lines = [f"Event A: {event_a}", f"  Position: {pos_a}", f"Event B: {event_b}", f"  Position: {pos_b}"]

                if num_a is not None and num_b is not None:
                    if num_a < num_b:
                        lines.append("\nTimeline: Event A → Event B (A occurs before B)")
                        lines.append("Verdict: ✓ No conflict — events are in chronological order.")
                    elif num_a > num_b:
                        lines.append("\nTimeline: Event B → Event A (B occurs before A)")
                        lines.append("Verdict: ✓ No conflict — events are in chronological order.")
                    else:
                        lines.append("\nTimeline: Same chapter position")
                        lines.append("Verdict: ⚠ Same position — check if these events can coexist.")
                elif num_a is not None or num_b is not None:
                    lines.append("\nVerdict: ⚠ Partial positioning — only one event has a chapter reference.")
                else:
                    lines.append(
                        "\nVerdict: ⚠ No chapter references provided. "
                        "Consider adding chapter_a/chapter_b for accurate checking."
                    )
            ...
            return _format_error("check_timeline_conflict", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("check_timeline_conflict", f"Unexpected error: {exc}")

    return [create_outline, edit_outline, delete_outline, check_timeline_conflict]
...
def create_foreshadowing_tools(project_id: str) -> list[Any]:
    """Create foreshadowing management and detection tools bound to a project_id.
    ...
    def create_foreshadowing(
        ...
        Use this tool when the story introduces an element that will pay off later.
        Examples: a seemingly insignificant item, a mysterious character, an offhand comment.

        Args:
            description: What is being foreshadowed (the setup)
            expected_resolve_at: When/where this should resolve (chapter/arc reference)
            foreshadowed_at: Where this was introduced (chapter reference)
            target_chapter_id: ID of the chapter where this was foreshadowed
            notes: Additional context for future reference

        Returns:
            Confirmation message with the created foreshadowing ID
        """
        ...
        try:
            ...
            async def _create() -> dict[str, Any]:
                async with get_db() as db:
                    service = ContentService(db)
                    result = await service.create_foreshadowing(
                        project_id=project_id,
            ...
            asyncio.run(_create())
            return f"Successfully created foreshadowing entry '{description[:80]}' (ID: {entry_id})."
        except (sqlite3.Error, RuntimeError) as exc:
            return _format_error("create_foreshadowing", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("create_foreshadowing", f"Unexpected error: {exc}")

    @tool
    def list_foreshadowing(status: str = "") -> str:
        """List foreshadowing entries, optionally filtered by status.

        Status values: "pending", "active", "resolved", "abandoned", or empty for all.

        Args:
            status: Optional status filter (pending/active/resolved/abandoned)

        Returns:
            Formatted list of foreshadowing entries
        """
        try:
            ...
            async def _list() -> list[dict[str, Any]]:
                async with get_db() as db:
                    ...
                    results = await service.list_foreshadowing(project_id, status=effective_status)
                return results

            results = asyncio.run(_list())
            if not results:
                return "No foreshadowing entries found."
            lines = [f"Foreshadowing entries ({len(results)} total):"]
            for r in results:
                desc = r.get("description", "No description")[:80]
                entry_status = r.get("status", "unknown")
                lines.append(f"- [{entry_status}] {desc} (ID: {r.get('id', 'N/A')})")
            return "\n".join(lines)
        except (sqlite3.Error, RuntimeError) as exc:
            return _format_error("list_foreshadowing", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("list_foreshadowing", f"Unexpected error: {exc}")

    @tool
    def resolve_foreshadowing(entry_id: str) -> str:
        """Mark a foreshadowing entry as resolved (payoff delivered).

        Args:
            entry_id: ID of the foreshadowing entry to resolve
        ...
        try:
            ...
            async def _resolve() -> dict[str, Any] | None:
                async with get_db() as db:
                    service = ContentService(db)
                    return await service.resolve_foreshadowing(entry_id, project_id)

            result = asyncio.run(_resolve())
            if not result:
                return _format_error("resolve_foreshadowing", f"Entry '{entry_id}' not found")
            return f"Successfully resolved foreshadowing entry '{entry_id}'."
        except (sqlite3.Error, RuntimeError) as exc:
            return _format_error("resolve_foreshadowing", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("resolve_foreshadowing", f"Unexpected error: {exc}")

    @tool
    def delete_foreshadowing(entry_id: str) -> str:
        """Delete a foreshadowing entry.

        Args:
            entry_id: ID of the foreshadowing entry to delete
        ...
        try:
            ...
            async def _delete() -> bool:
                async with get_db() as db:
                    service = ContentService(db)
                    return await service.delete_foreshadowing(entry_id, project_id)

            result = asyncio.run(_delete())
            if not result:
                return _format_error("delete_foreshadowing", f"Entry '{entry_id}' not found")
            return f"Successfully deleted foreshadowing entry '{entry_id}'."
        except (sqlite3.Error, RuntimeError) as exc:
            return _format_error("delete_foreshadowing", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("delete_foreshadowing", f"Unexpected error: {exc}")

    @tool
    def get_unresolved_foreshadowing() -> str:
        """Get all unresolved (pending or active) foreshadowing entries.

        Use this tool before writing new chapters to ensure unresolved
        foreshadowing elements are properly addressed.

        Returns:
            Formatted list of unresolved foreshadowing entries
        """
        try:
            ...
            async def _get() -> list[dict[str, Any]]:
                async with get_db() as db:
                    service = ContentService(db)
                    results = await service.get_unresolved_foreshadowing(project_id)
                return results

            results = asyncio.run(_get())
            if not results:
                return "No unresolved foreshadowing entries."
            lines = ["Unresolved foreshadowing:"]
            for r in results:
            ...
            return _format_error("get_unresolved_foreshadowing", f"Database error: {exc}")
        except Exception as exc:
            return _format_error("get_unresolved_foreshadowing", f"Unexpected error: {exc}")

    @tool
    def detect_foreshadowing(content: str, chapter_title: str = "") -> str:
        """Analyze chapter content to automatically detect potential foreshadowing.

        Call this tool AFTER writing a chapter. It uses AI to identify descriptions,
        objects, events, or character actions that may serve as foreshadowing,
        and automatically creates foreshadowing entries for them.

        Args:
            content: The chapter content to analyze for potential foreshadowing
            chapter_title: Optional chapter title for context

        Returns:
            Summary of detected and auto-created foreshadowing elements
        """
        try:

            async def _detect_and_save() -> str:
                async with get_db() as db:
                    service = ContentService(db)
                    llm = create_llm_client(streaming=False)

                    prompt = (
                        "You are a foreshadowing detection expert for novel writing. "
                        "Analyze the following chapter content and identify elements that "
                        "appear to be intentional foreshadowing — descriptions, objects, events, "
                        "character actions, or dialogue that seem designed to pay off later.\n\n"
                        "For each potential foreshadowing, provide:\n"
                        "1. description: A clear description of the foreshadowing element\n"
                        "2. reasoning: Why this seems like foreshadowing\n"
                        "3. confidence: HIGH, MEDIUM, or LOW\n\n"
                        "Output ONLY a JSON array of objects with keys: description, reasoning, confidence. "
                        "If nothing seems like foreshadowing, output an empty array [].\n\n"
                        f"Chapter title: {chapter_title or 'Untitled'}\n\n"
                        f"Content:\n{content[:8000]}"
                    )
                    ...
                    detected = _parse_detection_result(text)

                    if not detected:
                        return "No potential foreshadowing detected in the chapter."

                    # Auto-create foreshadowing entries for high-confidence detections
                    created = []
                    for item in detected:
                        ...
                        if not description:
                            continue
                        notes = f"Auto-detected ({confidence} confidence). Reasoning: {item.get('reasoning', '')}"
                        await service.create_foreshadowing(
                            project_id=project_id,
                            description=description,
                            foreshadowed_at=chapter_title or "Detected chapter",
                            notes=notes,
                        )
                        created.append(f"[{confidence}] {description}")

                    if not created:
                        return "Foreshadowing candidates identified but none met the threshold for auto-creation."

                    result_lines = [f"Auto-detected {len(created)} potential foreshadowing element(s):"]
                    result_lines.extend(f"  - {c}" for c in created)
                    return "\n".join(result_lines)

            return asyncio.run(_detect_and_save())
        except Exception as exc:
            return _format_error("detect_foreshadowing", f"Detection failed: {exc}")
...
def create_all_tools(project_id: str) -> list[Any]:
    ...
    tools.extend(create_foreshadowing_tools(project_id))
    return tools
