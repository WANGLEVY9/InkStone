...
from app.core.agent.novel_context_middleware import NovelContextMiddleware
...
def create_orchestrator_graph(
    ...
    # Pass checkpointer to enable state persistence for sub-agents
    world_builder = create_world_builder_agent(project_id, checkpointer=checkpointer)
    character = create_character_agent(project_id, checkpointer=checkpointer)
    plot = create_plot_agent(project_id, checkpointer=checkpointer)
    chapter = create_chapter_agent(project_id, checkpointer=checkpointer)
    review = create_review_agent(project_id, checkpointer=checkpointer)
    ...
    # Novel context middleware (appends novel-specific context to system prompt)
    novel_middleware = NovelContextMiddleware(novel_context=novel_context)
    ...
    return create_agent(
        ...
        middleware=[skill_middleware, novel_middleware],
    )
