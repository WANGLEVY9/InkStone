...
AI-powered web novel generation platform ("砚台") that uses LangChain/LangGraph to orchestrate multiple specialized sub-agents (world-builder, character, plot, chapter, review) under a single supervisor. Long-form content is generated with consistency tracking across world-building, characters, plot, and chapters.
...
| UI Library | Ant Design 6 (`antd`) + `@ant-design/x` for chat |
...
[backend/app/core/graph/builder.py](backend/app/core/graph/builder.py) uses `langchain.agents.create_agent` to build the orchestrator. Each sub-agent is wrapped as a `@tool` function (`delegate_to_world_builder`, `delegate_to_character`, etc.) that calls `subagent.invoke()` and returns the result. The orchestrator LLM decides which delegate tool to call based on the user's request.
...
[backend/app/core/agent/skill_middleware.py](backend/app/core/agent/skill_middleware.py) implements LangChain's `AgentMiddleware` to inject skill descriptions into the orchestrator's system prompt at runtime. Skills are markdown files with YAML frontmatter stored in `backend/data/skills/`, managed by [SkillService](backend/app/services/skill.py). The `SkillMiddleware` reads skills from disk on each model call, so newly created skills are automatically visible without rebuilding tools. The `load_skill` tool (registered via middleware) lets the agent load a skill's full content on demand. Sub-agents receive domain-filtered skills via `SkillMiddleware(domain="chapter")` etc.
...
The chat endpoint uses **LangGraph v2 streaming** (`graph.astream` with `stream_mode=["messages", "updates", "custom"]` and `subgraphs=True`). SSE event types:

| Event | Purpose |
|-------|---------|
| `messages` | LLM tokens with `source` attribution (orchestrator vs sub-agent name), plus `thinking` blocks |
| `updates` | Tool start/end events (`tool_start` / `tool_end`) |
| `custom` | Progress updates from sub-agents |
| `done` | Stream completion with aggregated `full_content` and `full_thinking` |
| `error` | Error details with `error_code`, `message`, `retryable` |

The frontend [useProjectChat.ts](frontend/src/hooks/useProjectChat.ts) parses these SSE events into a timeline of `ChatTimelineEvent` items. Sub-agent tokens (where `source != "orchestrator"`) are routed into the corresponding delegate tool's `subTimeline`, creating nested thinking/text displays. Tool categorization: `DELEGATE_TOOLS` (the 5 delegate_* tools) get expandable sub-timelines; `READ_ONLY_TOOLS` get a muted "read" badge.

History truncation uses `trim_messages` with token-based strategy (max 12K tokens), falling back to a 40-message count limit.
...
| Skills | `/skills` | [skills.py](backend/app/api/v1/skills.py) |
...
- [storage.py](backend/app/services/storage.py): `FileSystemStorage` with atomic writes and path-traversal protection for `.md` files in `backend/data/{project_id}/`
- [base.py](backend/app/services/base.py): `BaseContentService` with shared `_update_metadata()` helper
...
- [skill.py](backend/app/services/skill.py): `SkillService` manages skill files as YAML-frontmatter markdown in `backend/data/skills/`
- [content.py](backend/app/services/content.py): `ContentService` facade that composes the domain services (kept for backward compat)
...
Frontend implementation: [frontend/](frontend/) - React 18 + TypeScript + Vite + Ant Design 6 + `@ant-design/x`

### Key Design Decisions

- **Layout**: Classic sidebar layout using Ant Design `Layout` + `Sider` + `Menu`. `IconSidebar` (dark wood theme `#3A2A1F`) is always visible; `SecondaryNav` appears inside project pages.
- **AI Chat**: Available as both a floating draggable `Drawer` (Ctrl+J toggle, resizable) via [DrawerChat.tsx](frontend/src/components/ai/DrawerChat.tsx) and a full-page route at `/projects/:id/chat` via [ChatPage.tsx](frontend/src/pages/chat/ChatPage.tsx). Both use the shared [ChatCore.tsx](frontend/src/components/ai/ChatCore.tsx) component.
- **Chat UI**: Built with `@ant-design/x` components (`Bubble`, `Sender`, `Conversations`). Timeline renders nested thinking collapses, tool events with delegate sub-timelines, and error alerts.
- **SSE Streaming**: Raw `fetch()` + `ReadableStream` in [useProjectChat.ts](frontend/src/hooks/useProjectChat.ts). Uses ref+mutation pattern (`messagesRef`) for streaming state updates — NOT functional `setState` — to avoid stale closure issues in tight read loops.
...
- **Markdown Editor**: `vditor` for world/character/chapter editing
- **Theming**: Ant Design `ConfigProvider` with Design Tokens — Chinese literary palette (crimson primary `#C8323D`, vintage paper backgrounds `#F5EFE3`/`#FBF6EA`, serif font family `Noto Serif SC`)
...
```
frontend/src/
├── api/                    # Axios API clients per domain (client.ts, projects.ts, sessions.ts, etc.)
├── components/
│   ├── layout/             # AppLayout, IconSidebar, SecondaryNav, DashboardLayout
│   ├── ai/                 # ChatCore, DrawerChat, WelcomeScreen
│   ├── common/             # MarkdownEditor, PageHeader, EmptyState, PageContainer
│   ├── cards/              # ProjectCard, WorldCard, CharacterCard
│   └── dashboard/          # BookshelfView, ListView, LeftPanel, RightPanel, ControlBar
├── pages/                  # Route-level page components (world/, characters/, outline/, chapters/, chat/, settings/)
├── hooks/                  # Custom hooks (useProjectChat, useAppMessage, useVditor)
...
| `/projects/:id/chat[/:sessionId]` | ChatPage | Full-page AI chat |
...
```bash
python start-dev.py                              # Starts backend (8000) + frontend (5173)
```
...
### Docker
```bash
docker compose up                                # Starts frontend (:3000) + backend (:8001)
```
Both [frontend/Dockerfile](frontend/Dockerfile) and [backend/Dockerfile](backend/Dockerfile) are orchestrated via [docker-compose.yml](docker-compose.yml).
...
`backend/tests/` (23 test files). Notable patterns:
...
- `test_v2_streaming.py` - V2 streaming protocol
- `test_skill_middleware.py`, `test_skill_service.py`, `test_skill_tools.py`, `test_skills_api.py` - Skill system

`pytest.ini_options.asyncio_mode = "auto"` so `async def test_...` works without explicit markers.

## Repository-Specific Conventions

- **No mocks/stubs without authorization**: When you can't implement something, raise `NotImplementedError` rather than silently returning fake data.
...
├── api/v1/                       # 8 routers (see API table above)
├── core/
│   ├── agent/
...
│   │   ├── skill_middleware.py   # SkillMiddleware - AgentMiddleware for runtime skill injection
│   │   └── skill_tools.py        # create_skill tool factory
│   ├── graph/
│   │   ├── builder.py            # create_orchestrator_graph - subagents-as-tools wiring
│   │   └── state.py              # OrchestratorState, ProjectContext TypedDicts
│   ├── prompts/
│   │   ├── orchestrator.py       # ORCHESTRATOR_SYSTEM prompt
│   │   └── tools/                # Domain-specific prompt templates (chapter, character, plot, review, world)
...
│   │   ├── common/             # MarkdownEditor, PageHeader, EmptyState, PageContainer
│   │   ├── cards/              # ProjectCard, WorldCard, CharacterCard
│   │   └── dashboard/          # BookshelfView, ListView, LeftPanel, RightPanel
│   ├── pages/                  # Route-level page components
│   ├── hooks/                  # useProjectChat, useAppMessage, useVditor
│   ├── contexts/               # ProjectContext, ChatContext
...
docs/                            # Requirements, design docs, evaluation reports
```
